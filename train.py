...
# EGGROLL hyperparameters (can be overridden by config file)
pop_size = 1024
rank = 1
sigma = 0.01
alpha = 0.1
...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  bias=bias, vocab_size=None, dropout=dropout, sigma=sigma, rank=rank) # added sigma,rank
if init_from == 'scratch':
    ...
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias', 'vocab_size', 'sigma', 'rank']:
...
while True:
    ...
    # fetch next batch for next iteration (as in original)
    X, Y = get_batch('train')
